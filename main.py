    ...
    if os.path.exists(video_path):
        ...
        while current_retry < max_retries:
            try:
                ...
                result = subprocess.run(["./yt-dlp", "--format", "best", "-o", "--abort-on-unavailable-fragment", "-N 10", video_path, text], capture_output=True, text=True, check=True)
        ...
        if os.path.exists(video_path):
            ...
            while current_retry < max_retries:
                try:
                    ...
                    result = subprocess.run(["./yt-dlp", "--format", "best", "-o", "--abort-on-unavailable-fragment", "-N 10", video_path, link], capture_output=True, text=True, check=True)
        ...
        if os.path.exists(music_path):
            ...
            while current_retry < max_retries:
                try:
                    ...
                    result = subprocess.run(["./yt-dlp", "--extract-audio", "--audio-format", "mp3", "--ffmpeg-location", ffmpeg_location, "-o", "--abort-on-unavailable-fragment", "-N 10", music_path, link], capture_output=True, text=True)
                    output = result.stdout.strip() if result.stdout else result.stderr.strip()
